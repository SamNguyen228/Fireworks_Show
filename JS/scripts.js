...
const horsetailShellII = (size=1) => {
	const color = randomColor();
	const explode = Math.random() < 0.6;
	const crossette= Math.random() < 0.6;
	return {
		shellSize: size,
		horsetail: true,
		color,
...
const starShell = (size=1) => {
	const color = randomColor();
	const starShape = Math.random() < 0.6;
	const starShape2 = Math.random() < 0.8;
	...
	return {
		...
		starShape2,
		color,
		spreadSize: 260 + size * 60,
		starLife: 600 + size * 200,
		starLifeVariation: 1,
		starCount: 12 * PI_2 * (size + 2),
		streamers: Math.random() < 0.3,
		strobe: true,
		strobeColor: Math.random() < 0.5 ? COLOR.White : null,
	};
...
const colorShell = (size=1) => {
	const floral = Math.random() < 0.6;
	const starstorm = Math.random() < 0.5;
	const blooming = Math.random() < 0.8;
	return {
		shellSize: size,
		spreadSize: 180 + size * 90,
		starDensity: 0.5,
		starLife: 1100 + size * 300,
		starLifeVariation: 0.8,
		glitter: 'medium',
		glitterColor: COLOR.Gold,
		color: Math.random() < 0.65 ? 'random' : (Math.random() < 0.15 ? randomColor() : [randomColor(), randomColor({ notSame: true })]),
...
const crackleShellII = (size=1) => {
	const color = Math.random() < 0.5 ? COLOR.Gold : randomColor();
	return {
		shellSize: size,
		spreadSize: 380 + size * 75,
		starDensity: isLowQuality ? 0.65 : 1,
...
function startSequence() {
	...
	if (finaleSelector()) {
		...
		finaleShellCount++;
		console.clear();
		console.log(`Số pháo finale đã bắn: ${finaleShellCount}`);
	...
	if (rand < 0.6 && !IS_HEADER) {
		return seqRandomShell(), seqRandomFastShell()
	} else if (rand < 1) {
		return seqTriple(), seqTwoRandom();
	}
}
...
function bloomingFlowerEffect(star) {
	const count = 18;
	const layers = 5; 
	...
	for (let layer = 0; layer < layers; layer++) {
		const currentLayerCount = count + layer * 4;
		const speedMultiplier = baseSpeed + layer * layerDistance;

		createBurst(currentLayerCount, (angle, speedMult) => {
			Star.add(
				star.x,
				star.y,
				star.color,
	...
	BurstFlash.add(star.x, star.y, 60);
	soundManager.playSound('burstSmall');
}

// Radiant Burst Effect: A glowing effect with particles transitioning from blue to white.
function radiantBurstEffect(star) {
	const count = isHighQuality ? 40 : 20;
	createParticleArc(0, PI_2, count, 3, (angle) => {
		const speed = Math.pow(Math.random(), 0.6) * 3; 
		const life = 1200 + Math.random() * 600; 
		const color = Math.random() < 0.5 ? COLOR.Blue : randomColor(); // Randomly alternate colors
		Star.add(
			star.x,
			star.y,
			color,
		...
		if (typeof this.color === 'string') {
			...
			if (this.starShape) { 
				...
				createParticleArc(0, PI_2, this.starCount, 0, angle => {
					const radius = starRadius + waveAmplitude * Math.sin(waveFrequency * angle);
					const xStar = radius * Math.cos(angle);
					const yStar = radius * Math.sin(angle);
			
					const initSpeedX = -yStar * starScale * speed;
					const initSpeedY = -xStar * starScale * speed;
			...
			if (this.starShape2) { 
				...
				const waveSharpness = 3;
			
				createParticleArc(0, PI_2, this.starCount, 0, angle => {
					const radius = starRadius * (1 + Math.pow(Math.abs(Math.sin(waveFrequency * angle)), waveSharpness));
					const xStar = radius * Math.cos(angle);
					const yStar = radius * Math.sin(angle);
